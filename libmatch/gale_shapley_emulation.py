...
def gs_cs_match(students, courses):
    ...
    model.objective = maximize(xsum(list(var * 10000 for var in all_vars) + list(student_to_course_to_vars[s][c] * (-s.preferences.index(c)) for s in students for c in courses)))
    ...
    for student in students:
        for course in courses:
            ...
            model += (course.capacity)*(1-xsum(student_to_course_to_vars[student][cou] for cou in better_courses)) <= xsum(student_to_course_to_vars[stu][course] for stu in better_students)
